...
def simulate_smo(lambda1, lambda2, mu, rang, max_requests=1000):
    ...
    while processed < max_requests:
        ...
        if ARRIVAL in event:
            queue.append((current_time, event))
            ...
            if not server_busy:
                ...
                if arrival_type == ARRIVAL_1:
                    wait_times1.append(0)
                else:
                    wait_times2.append(0)
            ...
            if queue:
                ...
                if arrival_type == ARRIVAL_1:
                    wait_times1.append(wait)
                else:
                    wait_times2.append(wait)
    ...
    avg_wait1 = sum(wait_times1) / len(wait_times1) if wait_times1 else 0
    avg_wait2 = sum(wait_times2) / len(wait_times2) if wait_times2 else 0
    avg_system = sum(system_times) / len(system_times)

    R_fact = busy_time / current_time

    return R_calc, R_fact, avg_wait1, avg_wait2, avg_system, current_time

if __name__ == '__main__':
    _, _, avg_wait1, avg_wait2, _, _ = simulate_smo(0.35, 0.3, 2, 3, 1000)  
    print(f"avg_wait1 = {avg_wait1} \navg_wait2 = {avg_wait2}")
